...
export class LambdaFunction {
  static generateFunctionProps(props: LambdaProps) : lambda.FunctionProps {
    return {
      functionName: `${props?.vars.project}-gauge-mutation-${props?.vars.env}-lambda-function`,
      role: props?.lambdaExecutionRole,
      runtime: lambda.Runtime.JAVA_21,
      handler: 'org.springframework.cloud.function.adapter.aws.FunctionInvoker::handleRequest',
      code: lambda.Code.fromAsset(`assets/${props?.vars.env}/gauge-0.0.1-SNAPSHOT-aws.jar`),
  ...
  static generateAliasOptions() : { name: string, options: lambda.AliasOptions } {
    return {
      name: 'live',
      options: {
        description: 'Alias for gauge mutation'
      }
    };
  };
